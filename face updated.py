...
for label, person in enumerate(os.listdir(KNOWN_DIR)):
    ...
    for image_file in os.listdir(person_dir):
        ...
        predicted = recognize_face(image, embedding_database, threshold=0.9)
        y_true.append(person.strip().lower())
        y_pred.append(predicted.strip().lower())

# Accuracy
accuracy = accuracy_score(y_true, y_pred)
print(f"\n✅ Model Accuracy: {accuracy * 100:.2f}% on {len(y_true)} samples")
