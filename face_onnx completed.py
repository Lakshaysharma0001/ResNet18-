...
onnx_session = ort.InferenceSession("resnet18_face.onnx")
...
def image_to_embedding(image):
    ...
    embedding = onnx_session.run(None, {"input": image})[0]
    embedding /= np.linalg.norm(embedding)
    return embedding[0]

def generate_avg_embedding_onnx(image_paths):
    import cv2
    embeddings = []
...
def generate_avg_embedding_onnx(image_paths: list, model) -> np.ndarray:
...
# Create data base
database = {name:generate_avg_embedding_onnx(paths, model=model) for paths, name in tqdm(zip(filepaths, os.listdir(cropped_root_path)), desc="Generating Embeddings")}
