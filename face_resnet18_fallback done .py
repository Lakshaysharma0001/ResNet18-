...
def generate_avg_embedding_arcface(image_paths, embedding_obj):
    embeddings = []
    for img_path in image_paths:
        try:
...
database = {
    name: generate_avg_embedding_resnet18(paths) for paths, name in tqdm(zip(filepaths, person_names), desc="Generating Embeddings")
}
...
def recognize_face(image: np.ndarray, database: dict, threshold: float = 1.0, model = None) -> str: # Changed default to None
    ...
    embedding_model_to_use = model if model is not None else resnet18

    image_emb = resnet18_embedding(image) # Use the correct embedding model
...
def recognize_face_with_embedding_obj(embedding_obj, database, threshold):
    try:
...
def recognize_face_with_embedding_obj_and_print(embedding_obj, database, threshold):
    try:
        if not embedding_obj:
            return "Unknown"
        test_embedding = np.array(embedding_obj[0]["embedding"])
...
def recognize_face(img, database, threshold=1.0):
    try:
        # Assuming you have a function to get embedding from image
        # Replace with your actual embedding function
        embedding_obj = resnet18_embedding(img)

        if embedding_obj is None:
            return "Unknown"

        test_embedding = np.array(embedding_obj)
        min_dist = float('inf')
        identity = "Unknown"
