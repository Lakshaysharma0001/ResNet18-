...
def resnet18_embedding(image):
    try:
        ...
        img_tensor = transform_resnet(image).unsqueeze(0)
        with torch.no_grad():
            emb = resnet18(img_tensor).numpy()
...
def generate_avg_embedding_resnet18(image_paths):
    import cv2
    import numpy as np
    embeddings = []
...
def generate_avg_embedding_resnet18(image_paths: list, model) -> np.ndarray:
    ...
    for image_path in image_paths:
        ...
        embedding = resnet18_embedding(image)
...
# Create data base
database = {name:generate_avg_embedding_resnet18(paths, model=model) for paths, name in tqdm(zip(filepaths, os.listdir(cropped_root_path)), desc="Generating Embeddings")}
...
def recognize_face(image: np.ndarray, database: dict, threshold: float = 1.0, model = model) -> str:
    ...
    image_emb = resnet18_embedding(image)
